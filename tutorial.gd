...
const STEP_COUNT := 5
## The lesson foe is the street boxer: it reads as the game's real fast melee
## opponent instead of the greybox mannequin, so the drill the player practises
## on is the same silhouette they meet on the first floor, only weakened.
const ENEMY_VARIANT_SCRIPT = preload("res://enemy_variant.gd")
const ENEMY_VARIANT := &"boxer"
const ENEMY_MAX_HEALTH := 45.0
...
## Where both exit paths land. Overriding or observing these two members is the
## only hook a harness needs: the skip button, the finish button and the headless
## test all leave through _skip_to_first_floor().
var exit_scene_path := "res://floor_one.tscn"
## Optional hand-off override, injected by a harness that cannot let the real
## scene change happen. Empty in the shipped game, where the tree switch runs.
var exit_requested := Callable()
...
## Swap the inherited greybox training mannequin for a weakened street boxer.
## The boxer brings the real fast-melee silhouette and its own punch clips, so
## what the player learns to dodge here is what they meet on the first floor.
## Its optical cloak is switched off: a lesson opponent has to be readable from
## the first frame of the combat drill, not fade in mid-approach.
func _use_lesson_enemy() -> void:
	var previous_enemy := enemy
	remove_child(previous_enemy)
	previous_enemy.queue_free()
	enemy = ENEMY_VARIANT_SCRIPT.new()
	enemy.name = "TutorialBoxer"
	enemy.configure_variant(ENEMY_VARIANT)
	enemy.boxer_cloak_enabled = false
	enemy.spawn_position = enemy_position
	enemy.visible = false
	add_child(enemy)
	# The base _ready() forces PROCESS_MODE_PAUSABLE, so the foe is parked only
	# after it entered the tree. The lesson numbers are applied *after* the
	# variant's reset, because that reset re-applies the street boxer's own
	# health and damage and would otherwise overwrite them.
...
## The lesson foe hits for less than the street boxer so a beginner survives
## three or four mistakes while still learning to read the telegraph. It must run
## *after* `reset_enemy()`: the variant's reset re-applies the street boxer's
## 55 HP and 12 damage, so writing the lesson numbers first would be pointless.
func _weaken_lesson_enemy() -> void:
	if not is_instance_valid(enemy):
		return
	enemy.max_health = ENEMY_MAX_HEALTH
	enemy.attack_damage = ENEMY_ATTACK_DAMAGE
	# The reset filled health from the street maximum, so the lesson maximum has
	# to be mirrored into the current health too, or the drill would start with a
	# full bar that only drains to 45.
...
func _show_step(next_step: int) -> void:
	...
	match step:
		...
		4:
			...
			# The attack hand is the last scripted hand, and the player is now in a
			# live fight: normal refill returns here, not one step later, or a board
			# that has spent its three punches would never draw a fourth.
			_restore_deck_rules()
		5:
...
func _show_combat_enemy() -> void:
	...
	# Re-applied after the reset rather than before it: the variant's own reset
	# writes its street numbers over the lesson ones.
	enemy.reset_enemy()
	_weaken_lesson_enemy()
...
## Hand back the deck's normal refill rules. `_lock_hand` parks every slot at
## FROZEN_REFILL_DELAY, so this has to clear both the deck-wide delay and the
## per-slot timers — otherwise a slot emptied after the lesson would sit at
## ~9999 s and the player would never draw a replacement card.
...
func _skip_to_first_floor() -> void:
	if exit_requested.is_valid():
		exit_requested.call(exit_scene_path)
		return
