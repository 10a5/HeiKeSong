...
## snapshot; this node never reads a player, deck, or filesystem telemetry. A
## credential is read only from the explicitly configured environment variable
## or key file at request time.
...
## The checked-out demo can use the local key.txt supplied by the developer.
## It is ignored by git and is read only at request time. Exported builds can
## instead put a token at user://evomap.key or provide api_key_env.
const KEY_PATHS := ["user://evomap.key", "res://key.txt"]
const DEFAULT_EVOMAP_ENDPOINT := "https://api.evomap.ai/v1/chat/completions"
const DEFAULT_EVOMAP_MODEL := "evomap-gemini-3.1-pro-preview"
...
func reload_configuration() -> bool:
	...
	for path: String in KEY_PATHS:
		if FileAccess.file_exists(path):
			config_path = path
			return configure({
				"provider": "openai_compatible",
				"endpoint": DEFAULT_EVOMAP_ENDPOINT,
				"model": DEFAULT_EVOMAP_MODEL,
				"key_file": path,
				# The public EvoMap curl contract does not require this optional
				# OpenAI field; keep JSON enforcement in the system prompt instead.
...
func configure(config: Dictionary) -> bool:
	## Safe to call before add_child(). API credentials use an environment
	## variable or a local key file; a literal api_key field is deliberately
	## rejected so secrets are never copied into a saved config dictionary.
	if busy:
	...
	if config.has("api_key"):
		return _fail("请使用 api_key_env 或 key_file 指定密钥，不在配置中填写密钥")
	...
	var key_file: Variant = config.get("key_file", "")
	if not key_file is String:
		return _fail("key_file 必须是 res:// 或 user:// 路径")
	key_file = str(key_file).strip_edges()
	if not str(key_file).is_empty() and not _is_secret_path(str(key_file)):
		return _fail("key_file 必须是 res:// 或 user:// 路径")
	if not str(env_name).is_empty() and not str(key_file).is_empty():
		return _fail("api_key_env 与 key_file 只能指定一个")
	...
	if config.has("response_format") and not config["response_format"] is bool:
		return _fail("response_format 必须是布尔值")
	_config = {
		"provider": provider, "endpoint": endpoint, "model": model,
		"api_key_env": str(env_name),
		"key_file": str(key_file),
		"response_format": bool(config.get("response_format", true)),
...
func request_summary(context: Dictionary) -> bool:
	...
	var credential := _read_credential()
	if not bool(credential.get("ok", false)):
		return _fail(str(credential.get("error", "LLM 密钥无效")))
	var api_key := str(credential.get("value", ""))
	if not api_key.is_empty():
		# The file already contains the complete bearer token (sk-evomap-...).
		# Never prepend a provider prefix here.
	...
	if _config["provider"] == "ollama":
		payload["format"] = "json"
	elif bool(_config.get("response_format", true)):
...
func _is_secret_path(path: String) -> bool:
	## Restrict file credentials to Godot's two sandbox/resource schemes. This
	## keeps a user-supplied config from turning the bridge into an arbitrary
	## filesystem reader, while still supporting the checked-out key.txt.
	if path.is_empty() or path.length() > 256 or path.contains("\n") or path.contains("\r"):
		return false
...
func _valid_credential(value: String) -> bool:
	if value.is_empty() or value.length() > 512 or value.contains("\n") or value.contains("\r"):
		return false
	# API tokens are opaque ASCII strings. Reject spaces and control characters
	# without logging or exposing the token itself.
	var pattern := RegEx.new()
	pattern.compile("^[A-Za-z0-9._~+-]{16,512}$")
	return pattern.search(value) != null


func _read_credential() -> Dictionary:
	var key_file := str(_config.get("key_file", ""))
	if not key_file.is_empty():
		if not _is_secret_path(key_file) or not FileAccess.file_exists(key_file):
			return {"ok": false, "error": "LLM key_file 不存在或路径无效"}
		var file := FileAccess.open(key_file, FileAccess.READ)
		if file == null or file.get_length() > MAX_KEY_BYTES:
			return {"ok": false, "error": "LLM key_file 无法读取或超过大小限制"}
		var value := file.get_as_text().strip_edges()
		if not _valid_credential(value):
			return {"ok": false, "error": "LLM key_file 内容无效"}
		return {"ok": true, "value": value}
	var env_name := str(_config.get("api_key_env", ""))
	if env_name.is_empty():
		return {"ok": true, "value": ""}
	var value := OS.get_environment(env_name)
	if not _valid_credential(value):
		return {"ok": false, "error": "LLM 密钥环境变量未设置或内容无效"}
	return {"ok": true, "value": value}
