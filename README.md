...
一个可运行的 Godot 4.x **3D 灰盒训练场**，带可旋转与缩放的相机，使用 **11 种动作、初始 10 张／全解锁 18 张随机牌库、4 张可见手牌** 验证：**移动 → 选择动作牌 → 消耗能量 → 动作执行 → 弃牌与补牌 → 再出牌**。
...
项目现在用 Godot 原生 `AnimationPlayer` 驱动主角骨骼。主角使用 `main Character/futuristic+armored+female+3d+model (5).glb`，资源内嵌了 `idle`、`run`、`punch`、跳跃和前滚动画；移动时播放 `run`，停止后循环播放 `idle`。劈砍卡和突进挥砍卡现在映射到 `punch`，跳跃与翻滚使用模型动作，扫腿在模型没有专用片段时使用运行时生成的扫腿片段；如果以后加入同名或带有 `kick` / `扫腿` 的片段，会自动优先使用。运行时会移除 Hips 根位移，避免动画位移和玩家碰撞移动叠加。`character_visual.gd` 的 `use_animation_player` 可临时关闭以回到旧的程序化姿势。
...
左侧牌堆下方的“查看卡组”按钮可打开完整卡组，面板内可切换“全部卡组 / 抽牌堆 / 弃牌堆 / 动作图鉴”。同名牌分别展示，完整卡组包括当前手牌、抽牌堆与弃牌堆，并标注每张牌当前的位置；空牌堆有提示。牌面可以查看动作类型、费用和效果，卡牌较多时可滚动查看。抽牌堆按动作种类整理展示，实际仍随机抽取，列表顺序不代表抽牌顺序。“动作图鉴”展示全部十一种动作及解锁状态；未解锁条目只是图鉴，不计入实体牌数量。
...
认知受到扰乱，手牌代表当前能清晰想到的动作，所以同时只有四张。初始已恢复出拳、翻滚、突进出拳三种动作，分别拥有 4、3、3 张实体牌；随机抽四张到手中，其余六张留在抽牌堆。
...
每种新动作解锁一张牌，**立即进入抽牌堆**，无需重置。已经恢复的终端变暗，不能重复领取；手牌上限仍为四张。全部恢复后共有 **11 种动作、18 张实体牌**。按 R 会保留本次会话的解锁并重建整副牌；关闭游戏再启动会回到初始三种，本版尚未加入存档。
...
已实现 3D 灰盒场地、可操作相机、角色移动与平滑转身、跳跃与垂直运动、障碍物碰撞、十种动作的可解锁随机牌库、四手牌、场景记忆终端、翻滚同时攻击与空中连段、“爆发加速”初始义体，以及一个可战斗的训练敌人、双方生命、真实近战命中、翻滚避伤、暂停和重置。当前已加入自适应 Boss 的离线行为分析与本地反应原型；最终 Boss 的具体招式、幻境、奖励、存档和 LLM 台词层仍未接入。
