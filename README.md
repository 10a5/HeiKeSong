...
**当前默认进入随机生成的第一层水岸街区。** 地图已接入 `roguelike_map_handoff` 的噪声加权连通图：8×8 逻辑网格中分散放置 20 栋建筑，按种子选择 8 种旋转/镜像布局，并生成楼间地面路线与 8 场街道遭遇。每个建筑地块为紧凑的 7×5 块地砖（14×10 米）长方形，道路净宽 4 格。居民楼从现有 GLB 款式中按地图种子随机选择，工厂取代原写字楼，商店使用新的建筑模型；新模型继续支持遮挡时的蓝青科技渐变虚化。建筑使用贴合模型表面的简化碰撞，不再由整块空气墙包围；玩家可以靠近凹入的墙面，踏上模型里容得下角色的台阶和平台。普通移动可上下不高于 0.30 米的小台阶，较高障碍仍需跳跃。E 交互、M 地图、N 重新生成、R 同图重试。浅水可站立，越过一格浅水带进入深水后会下沉迷失。详细规则见 [第一层地图说明](docs/first-floor.md)。原训练场保留在 `main.tscn`；下面保留其操作与动作规则说明。
