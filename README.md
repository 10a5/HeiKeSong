...
**F5 默认先进入新手引导教学关（`tutorial.tscn`），完成后交接到随机生成的第一层水岸街区；击败镜像 Boss 后播放收尾演出（决战场崩塌 → 素子落水 → 水面俯视雾子与台词 → 数码商店闸门），再进入第二层与第三层。** 三层共用同一套代码，差别集中在 `level_stats.gd` 的一张表里：第二层是 160×160 米、第三层是 180×180 米（第一层 100×100 米），街道敌人生命 ×1.4 / ×1.85、伤害 ×1.25 / ×1.5，遭遇 10 / 12 场，镜像 Boss 生命 1000 / 1600（第一层 500）。道路生成规则、建筑尺度、遮挡、交互、水域、侵蚀、卡牌与义体、敌人前摇与判定窗口全部沿用第一层，因此战斗节奏不变、只是容错更低；详见 [第一层地图说明](docs/first-floor.md) 的“第二层与第三层”与 [Boss 战胜利演出](docs/floor-ending.md)。教学关用五个步骤依次教学移动、跳跃、出牌与“读抬手 → 翻滚 → 反击”，全程约 2～4 分钟，右上角随时可跳过直接进入第一层；设计说明见 [新手引导教学关说明](新手引导教学关_交付包/新手引导教学关说明.md)。第一层地图使用 `map_c_layout.gd` 的连续几何生长算法：在 100×100 米范围内先生成 68° 主干道和两侧支道，再沿道路边与已有建筑边放置模型，直到随机生长和 0.5 米收尾扫描都找不到合法位置。每个模型的俯视短边统一为 6 米，长边按其 GLB 实际比例计算，不固定建筑数量或旧的 14×10 米地块。道路、建筑碰撞、遮挡时的蓝青科技渐变虚化、E 交互、8 场街道遭遇和外围浅水/深水系统继续沿用现有第一层。详细规则见 [第一层地图说明](docs/first-floor.md) 和 [C 地图适配说明](docs/map-c.md)。原训练场保留在 `main.tscn`；下面保留其操作与动作规则说明。
...
2. 进入编辑器后按 **F5** 运行项目，默认从新手引导教学关开始（打开 `tutorial.tscn` 按 **F6** 可单独调试教学关）。教学完成后或点击右上角“跳过教学”即进入第一层；也可打开 `floor_one.tscn` 按 **F6** 直接运行第一层，`main.tscn` 可单独运行原训练场。想跳过教学做交付演示，把 `project.godot` 的 `run/main_scene` 改回 `res://floor_one.tscn` 即可。

要预览后续可用于场景切换的绿色数字雨效果，打开 `matrix_transition_preview.tscn` 后按 **F6**。预览会自动循环播放，按空格立即重播，按 Esc 退出。运行主游戏时按 **T** 可临时播放一次转场。

转场调用方法与外观参数见 [绿色数字雨说明](docs/matrix-transition.md)。
...
已加入最多五件的**义体配件**和可点击购买的商店。第一层贴近商店按 **E**，经数字雨转场后查看 **三张卡牌 + 三件配件**；商品从模糊逐渐显现。右侧可装备、卸下或出售配件，场外按 **I** 或点击“义体 x/5”管理装备。规则与价格见 [义体配件与商店](docs/implants-and-shop.md)。

| 输入 | 效果 |
| --- | --- |
| W / A / S / D 或上下左右方向键 | 相机相对的八方向移动，最后的移动方向决定角色朝向 |
| 空格 | 起跳；落地后可以再次跳跃，不消耗能量或卡牌 |
| 1–9 | 打出对应位置的当前手牌；默认四槽，义体可扩到九槽 |
| Q | 激活初始义体“爆发加速”；不消耗能量或手牌 |
| 点击手牌 | 与该卡槽数字键相同，执行卡片显示的动作 |
| 查看卡组按钮 / Tab | 打开完整卡组，查看每张牌的费用、作用与所在区域 |
| 义体按钮 / I | 查看装备与背包、当前强化，装备或卸下配件，最多装备五件 |
| 点击左侧抽牌堆 / 右侧弃牌堆 | 查看对应牌堆中的所有卡牌 |
| 在场景中按住鼠标左键拖动 | 旋转观察相机，松开停止；点击卡牌仍然出牌 |
| 触控板双指张开 / 捏合 | 张开拉近相机，捏合拉远相机 |
| 鼠标滚轮 / 触控板双指上下滑动 | 拉近 / 拉远相机 |
| E | 第一层贴近建筑交互，商店打开购买面板；训练场靠近动作记忆终端恢复动作 |
| R | 重置双方、生命和能量，保留本次解锁并重建牌组，重新发四张手牌 |
| Esc | 暂停 / 继续 |
| T | 预览绿色数字雨转场 |
| 击败镜像 Boss 后的演出中任意键 / 点击 | 跳过崩塌直接看台词，再按一次跳到数码闸门；此时 **R** 仍然中断演出并重试本图 |
...
认知受到扰乱，手牌代表当前能清晰想到的动作，所以同时只有四张。初始十张牌固定由 **3 张出拳、3 张生成护盾、2 张翻滚、1 张突进出拳、1 张回旋踢** 组成；每次从抽牌堆随机抽四张，其余六张留在抽牌堆。护盾牌费用 2，立即获得 10 点独立护盾，可叠加；护盾每 0.2 秒自然减少 1 点。
...
| 攻击 | 回旋踢 | 2 | 使用后 0.8 秒在自身周围 2 米造成 50 伤害；动作期间不能移动或出牌 |
| 攻击 | 扫腿 | 2 | 低身横扫前方，22 伤害 |
| 防御 | 生成护盾 | 2 | 获得 10 点独立护盾；每 0.2 秒失去 1 点 |
| 攻击 | 点射 | 2 | 朝面向直线射击，18 伤害，墙体会阻挡 |
| 攻击 | 蓄力重劈 | 3 | 蓄力约 0.20 秒后重击，45 伤害；可用位移牌取消 |
| 位移 | 翻滚 | 3 | 固定方向快速位移，0.44 秒动作时长、约 4.11 单位位移；可同时攻击 |
| 位移 | 短距闪现 | 3 | 向前闪现最多 3.5 米，胶囊碰撞防止穿墙；依靠位移躲避，无额外无敌 |
| 位移 | 喷射跃升 | 2 | 向上喷射并短暂前进，可衔接空中动作 |
| 复合 | 突进出拳 | 5 | 锁定方向突进，路径和末端出拳均可命中，45 伤害 |
...
敌人靠近后抬手，约 **0.3 秒后**落刀；这段需要闪避时才显示地面扇形刀锋范围，并在抬手期间固定攻击方向。命中扣 **20 生命**。在近战距离触发攻击后，单靠普通走路后退或横移来不及走出攻击；看到抬手后准备，在落刀前打出翻滚牌，利用位移和 **0.44 秒翻滚避伤窗口**躲开。过早翻滚、结束后还留在刀锋范围内仍会受伤。敌人收招约 **1.2 秒**，此时可以用劈砍或冲刺挥砍反击。
...
| `main.tscn` | 可直接运行的主场景 |
| `tutorial.tscn` / `tutorial.gd` | 新手引导教学关，F5 默认入口；五个步骤分别教学移动、跳跃、出牌与翻滚闪避 |
| `floor_one.tscn` / `floor_one.gd` | 第一层（100×100 米）与第二、三层的共用控制器：地图、服务、遭遇、水域、侵蚀与决战 |
| `floor_two.tscn` / `floor_three.tscn` | 第二层（160×160 米）与第三层（180×180 米），只声明层号，其余继承第一层 |
| `level_stats.gd` | 三层的地图边长、街道敌人生命/伤害倍率、遭遇数量与金币、镜像 Boss 生命/伤害倍率 |
| `enemy_variant.gd` | 街道敌人变体（大剑 / 拳击手 / 狙击手）；教学关复用 `boxer` 作为教学对手 |
| `新手引导教学关_交付包/` | 教学关原始交付包（脚本、场景与设计文档），保留备查 |
...
| `matrix_transition.gd` | 可复用的全屏绿色数字雨转场组件 |
| `matrix_transition_preview.tscn` | 数字雨转场独立预览场景 |
| `floor_ending.gd` | Boss 战胜利演出：决战场崩塌、落水、水面俯视雾子、数码商店闸门与下一层预载 |
| `ending_overlay.gd` | 演出的屏幕部分：电影黑边、台词“你我犹如隔镜视物，所见无非虚幻迷蒙”与数码闸门进度条 |
| `docs/floor-ending.md` | 胜利演出的阶段、时长、可调参数与实现说明 |
| `implant_catalog.gd` / `implant_inventory.gd` | 六种义体配件、独立库存和五件装备上限 |
| `shop_system.gd` / `shop_panel.gd` | 商店转场、三卡三配件展示、买卖和义体配置 |
| `docs/implants-and-shop.md` | 义体、商店价格、叠加规则与新局重置说明 |
