...
func _update_ai(delta: float) -> void:
	if combo_active:
		...
		if _step_wait_left <= 0.0 and not is_rolling and not is_dashing and slash_time_left <= 0.0:
			_combo_step += 1
...
func _apply_slash_hits() -> void:
	if is_entering or is_dead or not combat_enabled or slash_time_left <= 0.0 or get_tree().paused or not is_instance_valid(_actor) or bool(_actor.get("is_dead")):
		return
	...
	if bool(_actor.call("take_damage", _active_melee_damage)):
