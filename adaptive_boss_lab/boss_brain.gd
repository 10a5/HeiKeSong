...
func plan_reaction(player_state_override: Dictionary = {}) -> Dictionary:
	...
	if bool(state.get("is_rolling", false)):
		...
		if enough_evidence and float(patterns.get("roll_attack_rate", 0.0)) >= roll_attack_threshold and can_afford_attack:
			# The slow strategy layer must explicitly authorize the counter.
			# Evidence alone must not bypass the strategy -> FSM boundary.
...
func _infer_world_model() -> void:
	...
	for action in _action_history:
		...
		previous_kind = kind
		previous_time = float(action.get("time", _clock))
...
func _evaluate_reaction(immediate: bool) -> void:
	...
	# A card signal may request an urgent state change immediately, but repeated
	# identical requests are still coalesced. Physics ticks always respect the
	# cooldown, keeping the local FSM bounded under noisy telemetry.
