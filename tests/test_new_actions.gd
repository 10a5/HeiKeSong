...
func _test_front_kick_area() -> void:
	...
	_check(_near(enemy.health, 100.0), "Front kick damage waits for its delayed impact")
	...
	_check(_near(enemy.health, 100.0), "Front kick has not damaged before 0.8 seconds")
	await _steps(14)
	_check(_near(enemy.health, 100.0), "Front kick still waits during its recovery")
	await _steps(36)
	_check(_near(enemy.health, 50.0), "Front kick deals 50 area damage after 0.8 seconds")
	await _steps(4)
