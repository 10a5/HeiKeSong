...
func _test_decay() -> void:
	_reset()
	player.add_shield(10.0)
	player._update_shield_decay(0.19)
	_check(_near(player.shield, 10.0), "Shield remains unchanged before the first 0.2 seconds")
	player._update_shield_decay(0.01)
	_check(_near(player.shield, 9.0), "Exactly one shield point decays at 0.2 seconds")
	player._update_shield_decay(0.61)
	_check(_near(player.shield, 6.0), "A long frame applies every elapsed shield decay tick")
	player._update_shield_decay(0.19)
	_check(_near(player.shield, 5.0), "Decay preserves the fractional remainder between updates")
	player._update_shield_decay(0.01)
	_check(_near(player.shield, 5.0), "A fractional remainder reaches the next decay tick")
	_reset()
	player.add_shield(10.0)
	player._update_shield_decay(0.1)
	player.add_shield(10.0)
	player._update_shield_decay(0.1)
	_check(_near(player.shield, 19.0), "Adding shield to an existing shield does not reset its decay clock")
	...
	player.add_shield(10.0)
	player._update_shield_decay(0.19)
	_check(_near(player.shield, 10.0), "New protection after depletion begins a fresh 0.2-second interval")
...
func _test_physics_and_pause() -> void:
	...
	await _steps(12)
	_check(_near(player.shield, 9.0), "The live physics loop loses one shield point over twelve 60 Hz frames")
	paused = true
...
func _test_enemy_hit() -> void:
	...
	_check(_near(player.shield, 8.0), "A shielded enemy swing consumes its twenty damage only once")
	enemy.combat_enabled = false
	player.set_physics_process(false)
